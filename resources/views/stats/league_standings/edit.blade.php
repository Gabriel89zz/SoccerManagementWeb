...
<div class="card shadow" style="max-width: 900px; margin: 0 auto;">
    ...
    <div class="card-body">
        ...
        <form action="{{ route('league-standings.update', $standing->league_standing_id) }}" method="POST">
            @csrf @method('PUT')
                ...
                <div class="col-md-6">
                    ...
                    <select name="competition_season_id" class="form-select select2-search" required>
                        @foreach($compSeasons as $cs)
                            <option value="{{ $cs->competition_season_id }}" {{ $standing->competition_season_id == $cs->competition_season_id ? 'selected' : '' }}>
