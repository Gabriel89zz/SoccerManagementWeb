...
<div class="d-flex justify-content-between align-items-center mb-4">
    ...
    <a href="{{ route('league-standings.create') }}" class="btn btn-primary"><i class="fas fa-plus"></i> Add Entry</a>
</div>
                    ...
                    <tr>
                        <td>
                            @if($st->competitionSeason && $st->competitionSeason->competition)
                                <div class="fw-bold">{{ $st->competitionSeason->competition->name }}</div>
                                <div class="small text-muted">{{ $st->competitionSeason->season->name ?? '-' }}</div>
                            @else
                        ...
                        <td class="text-end">
                            <!-- BOTÓN SHOW -->
                            <a href="{{ route('league-standings.show', $st->league_standing_id) }}" class="btn btn-sm btn-info text-white" title="View Details">
                                <i class="fas fa-eye"></i>
                            </a>

                            <a href="{{ route('league-standings.edit', $st->league_standing_id) }}" class="btn btn-sm btn-warning" title="Edit">
                                <i class="fas fa-edit"></i>
                            </a>

                            <form action="{{ route('league-standings.destroy', $st->league_standing_id) }}" method="POST" class="d-inline delete-form">
                                @csrf @method('DELETE')
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        searchInput.addEventListener('input', function() {
            ...
            debounceTimer = setTimeout(() => {
                fetch(`{{ route('league-standings.index') }}?search=${query}`)
